...
/* 3. The bank is giving a new customer bonus! How do you update all records
	  with an account balance of 0.00 and a transactions_attempted of 0? Give them
	  a new account balance of 10.00.*/
UPDATE "songs" SET "published" = '1-1-1996' WHERE "id" = 1;
...
/* 5. How do you get the username and account balance of the 3 users with the
	highest balances, sorted highest to lowest balance? NOTE: Research LIMIT*/
SELECT "username" FROM "accounts" ORDER BY "account_balance" ASC LIMIT 3;
	
/* 6. How do you get the username and account balance of the 3 users with the
lowest balances, sorted lowest to highest balance?*/
...
transaction customers: How do you delete users that reside in miami OR phoenix
